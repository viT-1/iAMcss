...
3.0 в dark.css прописать `[text-editor-p]`
...
3.0 `iam-child = "{iam-parent}" iam-parent-child`
...
iAMcss 3.0 предлагает указывать блок и элемент всплошную вместе, разделяя только слова `block-name-elem-name`,
...
iAMCss 3.0: `[block-element *= "{--mod}"]` и `[block-element *= "{iam-root-block}{--mod}"]`
...
В нотациях iAMcss 2.0 и 3.0 именование блока размещено в виде префикса как часть именования атрибута (элемента).
...
В iAMcss 2.2 для указания блока как модификатора, использовать "^" (в 3.0 именование parent-атрибута как есть).
...
по iAMcss 3.0 `[iam-table-cell *= "--header"]`
