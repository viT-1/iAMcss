...
Блок/интерфейс не может иметь модификатор, тогда как контейнер блока может.
...
/* iAMcss 2.0 */
*[data-my-iTree__item][aria-expanded = 'true'] {}
*[data-my-iTreeTooltip]:hover {}
```
...
В целом каскад [не одобряется](https://ru.bem.info/faq/#Почему-нежелательно-использовать-вложенные-селекторы), хотя бы потому, что вам самим неудобно работать с селекторами
...
Запись ".class1 .class2" оправдана только для работы с контекстом состояния родительского
элемента. Если вы вынуждены громоздить такие цепочки только с целью повысить специфичность
селектора (перекрыть стили от фреймворка например), то значит уже что-то в вашем коде не так.

##Влияние селекторов кода сторонних фреймворков
Фреймворки (типа Bootstrap) навязывают использование своих глобальных имён классов.
...
В iAMcss 2.0 самое главное ограничение внешнее: названия атрибуты обязаны быть написаны
