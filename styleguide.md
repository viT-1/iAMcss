...
Большие различия по отображению принято собирать в файлы-шкурки/темы (skins).
...
но и версии, ориентация (горизонтальная/вертикальная) и прочее. Наборы значений переменных можно выделить в отдельный
файл и инициировать их как показано классом, либо `[iam-themes ~= some-theme]`, либо переключая файлы-шкурки с `:root {}`
...
## Префиксы
Валидными custom атрибутами считаются те, что начинаются с префиксов "data-".
...
[iam-list] {
	--iam-list--: ; /* контекст/переключатель */
}
